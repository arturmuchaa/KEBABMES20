/**
 * DeboningHmiV6Page — "Electric Dark"
 * Filozofia: absolutna ciemność, elektryczne akcenty, cyfry jako bohaterowie.
 * Inspiracja: SpaceX Dragon UI + Bloomberg Terminal + Linear.
 * Brak przełącznika motywu — ten ekran jest zawsze ciemny.
 */
...
// ─── Zegar izolowany ───────────────────────────────────────────────
const V6Clock = memo(function V6Clock() {
  ...
  return (
    <span style={{ fontFamily: 'monospace', fontSize: 22, fontWeight: 900, color: C.ink, letterSpacing: 1, tabularNums: true } as React.CSSProperties}>
...
// ─── Kafel partii ──────────────────────────────────────────────────
const V6BatchTile = memo(function V6BatchTile({ batch, selected, onSelect }: {
  ...
  return (
    <button type="button" onClick={() => onSelect(batch)}
      className="flex flex-col justify-between h-full text-left select-none transition-all duration-75 active:opacity-80"
      style={{
        background: selected ? C.acc : C.s1,
        borderLeft: `3px solid ${statusColor}`,
        padding: '10px 12px',
        borderRadius: 2,
        outline: selected ? `1px solid ${C.acc2}` : `1px solid ${C.bd}`,
        outlineOffset: 0,
      }}>
      <div className="flex items-start justify-between gap-2">
        <span style={{ fontFamily: 'monospace', fontSize: 20, fontWeight: 900, color: C.ink, lineHeight: 1 }}>
          {batch.internalBatchNo}
        </span>
        <span style={{ fontFamily: 'monospace', fontSize: 11, fontWeight: 700, color: selected ? 'rgba(255,255,255,0.85)' : statusColor }}>
          {daysLeft < 0 ? 'PRZET.' : daysLeft === 0 ? 'DZIŚ!' : `${daysLeft}d`}
        </span>
      </div>
      <span style={{ fontSize: 11, color: selected ? 'rgba(255,255,255,0.7)' : C.mut2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {supplierLabel}
      </span>
      <span style={{ fontFamily: 'monospace', fontSize: 11, color: selected ? 'rgba(255,255,255,0.6)' : C.mut2 }}>
...
// ─── Kafel pracownika ──────────────────────────────────────────────
const V6WorkerTile = memo(function V6WorkerTile({ worker, selected, entryCount, onSelect }: {
  ...
  return (
    <button type="button" onClick={() => onSelect(worker)}
      className="relative flex flex-col items-center justify-center gap-1.5 select-none transition-all duration-75 active:scale-[0.97]"
      style={{
        background: selected ? C.acc : C.s1,
        borderRadius: 2,
        outline: selected ? `1px solid ${C.acc}` : `1px solid ${C.bd}`,
      }}>
      <span style={{
        fontFamily: 'monospace', fontSize: 38, fontWeight: 900, lineHeight: 1,
        color: selected ? C.wht : C.ink,
      }}>{initials}</span>
      <span style={{
        fontSize: 12, fontWeight: 600, color: selected ? 'rgba(255,255,255,0.85)' : C.mut2,
      ...
      {entryCount > 0 && (
        <span className="absolute top-1.5 right-1.5 min-w-[20px] h-5 px-1 flex items-center justify-center rounded-sm text-[11px] font-black"
          style={{ background: selected ? 'rgba(255,255,255,0.25)' : C.grn, color: selected ? C.wht : '#000', fontFamily: 'monospace' }}>
          {entryCount}
        </span>
      )}
    </button>
...
// ─── Pole readout ──────────────────────────────────────────────────
function V6Readout({ label, value, active, unit = 'kg', helper, onActivate }: {
  ...
  return (
    <button type="button" onClick={onActivate} className="w-full text-left flex-shrink-0 flex flex-col"
      style={{
        ...
        borderLeft: `3px solid ${active ? C.acc : C.bd}`,
        borderRadius: 2,
        padding: '10px 14px',
        outline: `1px solid ${C.bd}`,
        transition: 'background 0.05s',
      }}>
      <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.18em', textTransform: 'uppercase', color: active ? C.acc : C.mut2 }}>
        {label}
      </span>
      <div className="flex items-baseline gap-2 mt-1">
        <span style={{ fontFamily: 'monospace', fontSize: 68, fontWeight: 900, lineHeight: 1, color: value ? C.ink : C.mut, letterSpacing: -1 }}>
          {value || '0'}
        </span>
        <span style={{ fontSize: 20, fontWeight: 700, color: C.mut2 }}>{unit}</span>
        {active && <span className="ml-auto animate-pulse" style={{ width: 3, height: 40, background: C.acc, borderRadius: 1, alignSelf: 'center' }} />}
      </div>
      {helper && <span style={{ fontSize: 12, color: C.mut2, fontFamily: 'monospace', marginTop: 2 }}>{helper}</span>}
    </button>
  )
}

// ─── Numpad ────────────────────────────────────────────────────────
const V6Numpad = memo(function V6Numpad({ onKey, onBsStart, onBsEnd, disabled }: {
  ...
  return (
    <div className={cn('flex flex-col gap-1.5 flex-1 min-h-0', disabled && 'opacity-30 pointer-events-none')}>
      {rows.map(row => (
        <div key={row[0]} className="flex gap-1.5 flex-1">
          {row.map(k => (
            <button key={k} type="button" onClick={() => onKey(k)}
              className="flex-1 flex items-center justify-center active:scale-[0.96]"
              style={keyStyle()}>
              {k}
            </button>
          ))}
      ...
      <div className="flex gap-1.5 flex-1">
        <button type="button" onClick={() => onKey('0')}
          className="flex-[2] flex items-center justify-center active:scale-[0.96]"
          style={keyStyle()}>0</button>
        <button type="button" onClick={() => onKey('.')}
          className="flex-1 flex items-center justify-center active:scale-[0.96]"
          style={{ ...keyStyle(), color: C.mut2 }}>·</button>
        <button type="button" onClick={() => onKey('⌫')}
          onPointerDown={onBsStart} onPointerUp={onBsEnd} onPointerLeave={onBsEnd}
          className="flex-1 flex items-center justify-center active:scale-[0.96]"
          style={{ ...keyStyle(), color: C.amb }}>
          <Delete size={28} />
        </button>
      </div>
    </div>
...
// ─── KPI bar ───────────────────────────────────────────────────────
const V6KpiBar = memo(function V6KpiBar({ entries, onEntries, onStats }: {
  ...
  const yColor = yPct >= 75 ? C.grn : yPct >= 60 ? C.amb : totMeat > 0 ? C.red : C.mut2
  ...
  return (
    <div className="flex-shrink-0 flex" style={{ height: 58, background: C.s1, borderTop: `1px solid ${C.bd}` }}>
      {cells.map(c => (
        <div key={c.label} className="flex-1 flex flex-col items-center justify-center"
          style={{ borderRight: `1px solid ${C.bd}` }}>
          <span style={{ fontFamily: 'monospace', fontSize: 22, fontWeight: 900, color: c.color, lineHeight: 1 }}>{c.val}</span>
          <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.16em', color: C.mut2, marginTop: 3 }}>{c.label}</span>
        </div>
      ))}
      <button type="button" onClick={onEntries}
        className="flex-1 flex flex-col items-center justify-center gap-0.5 active:opacity-70"
        style={{ borderRight: `1px solid ${C.bd}`, color: C.acc }}>
        <span style={{ fontSize: 18 }}>📋</span>
        <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.14em' }}>WPISY</span>
      </button>
      <button type="button" onClick={onStats}
        className="flex-1 flex flex-col items-center justify-center gap-0.5 active:opacity-70"
        style={{ color: C.grn }}>
        <BarChart3 size={18} />
        <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.14em' }}>STATYSTYKI</span>
      </button>
    </div>
  )
})
...
// ─── Główny komponent ──────────────────────────────────────────────
export function DeboningHmiV6Page() {
  ...
  if (!session) return (
    <div style={root}>
      <div className="flex flex-col flex-1 items-center justify-center gap-8">
        <div style={{ width: 96, height: 96, background: C.s2, border: `2px solid ${C.grn}`, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.grn }}>
          <Play size={52} />
        </div>
        ...
        <button type="button" onClick={handleStartDay} disabled={startLoading}
          ...
          style={{ height: 72, padding: '0 56px', background: C.grn, color: '#000', fontSize: 22, fontWeight: 900, borderRadius: 2, border: 'none', cursor: 'pointer' }}>
          {startLoading ? <span className="w-7 h-7 border-4 border-black/30 border-t-black rounded-full animate-spin" /> : <Play size={28} />}
          Rozpocznij dzień
        </button>
      </div>
    </div>
  ...
  if (session.status === 'closed' || session.status === 'approved') return (
    <div style={root}>
      <div className="flex flex-col flex-1 items-center justify-center gap-6">
        <div style={{ width: 96, height: 96, background: C.s2, border: `2px solid ${C.amb}`, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.amb }}>
          <Lock size={52} />
        </div>
  ...
  return (
    <div style={root}>
      ...
      <div className={cn('fixed top-3 left-1/2 -translate-x-1/2 z-50 px-5 py-3 flex items-center gap-3 font-bold transition-opacity duration-150', toastVis ? 'opacity-100' : 'opacity-0 pointer-events-none')}
        style={{ background: C.s3, border: `1px solid ${toastOk ? C.grn : C.red}`, color: toastOk ? C.grn : C.red, borderRadius: 2, fontSize: 14 }}>
        {toastOk ? '✓' : '⚠'} {toastMsg}
      </div>

      {/* NAGŁÓWEK */}
      <header className="flex-shrink-0 flex items-center gap-5 px-5" style={{ height: 60, background: C.s1, borderBottom: `1px solid ${C.bd}` }}>
        <span style={{ fontFamily: 'monospace', fontSize: 18, fontWeight: 900, letterSpacing: 3, color: C.ink }}>ROZBIÓR</span>
        <span style={{ fontSize: 12, color: C.mut2, fontFamily: 'monospace' }}>{session.sessionDate}</span>
        ...
        {[
          ...
          <div key={c.label} className="flex flex-col justify-center pl-4" style={{ borderLeft: `1px solid ${C.bd}` }}>
            <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.18em', color: C.mut2 }}>{c.label}</span>
        ...
        {fefoAlerts.length > 0 && (
          <span className="flex items-center gap-2 px-3 py-1" style={{ border: `1px solid ${C.red}`, color: C.red, borderRadius: 2, fontSize: 13, fontWeight: 700 }}>
        ...
        <button type="button" onClick={() => setShiftModal(true)}
          className="flex items-center gap-2 active:opacity-70" style={{ height: 34, padding: '0 14px', border: `1px solid ${C.bd}`, borderRadius: 2, background: 'transparent', color: C.mut2, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
          <LogOut size={14} /> Zakończ zmianę
        </button>
        <button type="button" onClick={() => setFinishModal(true)}
          className="flex items-center gap-2 active:opacity-70" style={{ height: 34, padding: '0 14px', border: `1px solid ${C.amb}`, borderRadius: 2, background: 'transparent', color: C.amb, fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
      ...
      <div className="flex-shrink-0 grid gap-1 px-1 py-1" style={{ height: 88, background: C.bg, gridTemplateColumns: `repeat(${Math.max(batches.length, 1)}, 1fr)` }}>
        {batchData.loading
          ...
          : batches.length === 0
            ? <div className="col-span-6 flex items-center justify-center" style={{ fontSize: 13, color: C.mut2 }}>Brak aktywnych partii</div>
            : batches.map(b => <V6BatchTile key={b.id} batch={b} selected={selBatch?.id === b.id} onSelect={pickBatch} />)
        }
      </div>
      ...
      <div className="flex-1 flex min-h-0">
          ...
          {workerData.loading
            ...
            : <div className="grid grid-cols-4 grid-rows-4 gap-1.5 h-full">
                {Array.from({ length: 16 }, (_, i) => {
                  ...
                  return <V6WorkerTile key={w.id} worker={w} selected={selWorker?.id === w.id} entryCount={entryCountByWorkerId.get(w.id) ?? 0} onSelect={pickWorker} />
                })}
              </div>
          }
        </div>
        ...
        <div className="flex-1 flex flex-col gap-2 p-3 min-h-0" style={{ background: C.bg }}>
          ...
          <div className="flex gap-2 flex-shrink-0">
            <V6Readout label="Ćwiartka kg" value={takenMode === 'kg' ? kgTaken : fmtKg(takenRaw * KG_PER_CONTAINER, 0)}
              active={active === 'taken' && takenMode === 'kg'} unit="kg"
              onActivate={() => switchTakenMode('kg')} />
            <V6Readout label="Pojemniki" value={takenMode === 'poj' ? kgTaken : String(Math.floor(takenRaw / KG_PER_CONTAINER) || '0')}
              active={active === 'taken' && takenMode === 'poj'} unit="poj."
              onActivate={() => switchTakenMode('poj')} />
          </div>

          {/* Pole mięsa */}
          <V6Readout label="Mięso Z/S" value={kgMeat} active={active === 'meat'} unit="kg"
            helper={yieldPct > 0 ? `${fmtPct(yieldPct, 1)} wydajność` : undefined}
            onActivate={() => setActive('meat')} />

          {/* Numpad */}
          <V6Numpad onKey={pressKey} onBsStart={onBsStart} onBsEnd={onBsEnd} disabled={!selBatch || !selWorker} />

          {/* ZAPISZ */}
          <button type="button" onClick={handleSave} disabled={!canSave || addLoading}
            className={cn('flex-shrink-0 flex items-center justify-center gap-4 active:scale-[0.99] transition-all', saveFlash && 'scale-[1.01]')}
            style={{
              height: 82, borderRadius: 2, fontSize: 22, fontWeight: 900, fontFamily: 'monospace', letterSpacing: 2,
              background: canSave ? (saveFlash ? '#00ff88' : C.grn) : C.s2,
              color: canSave ? '#000' : C.mut,
              border: `1px solid ${canSave ? C.grn : C.bd}`,
              boxShadow: canSave ? `0 0 28px rgba(0,230,118,0.3)` : 'none',
              opacity: !canSave ? 0.4 : 1,
              cursor: canSave ? 'pointer' : 'not-allowed',
              transition: 'background 0.1s, box-shadow 0.2s',
            }}>
            {addLoading ? <span className="w-8 h-8 border-4 border-black/30 border-t-black rounded-full animate-spin" /> : <Save size={30} />}
            ZAPISZ WPIS
          </button>
        </div>
      </div>
      ...
      {/* KPI BAR */}
      <V6KpiBar entries={entries} onEntries={() => setEntriesModal(true)} onStats={() => setStatsModal(true)} />

      {/* MODAL: Wpisy */}
      {entriesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="flex flex-col" style={{ width: 820, maxHeight: '85vh', background: C.s1, border: `1px solid ${C.bd2}`, borderRadius: 2 }}>
            <div className="flex items-center gap-4 px-6 py-4 flex-shrink-0" style={{ borderBottom: `1px solid ${C.bd}` }}>
              ...
              <span style={{ fontSize: 13, color: C.mut2, fontFamily: 'monospace' }}>{entries.length} wpisów</span>
              <button type="button" onClick={() => setEntriesModal(false)} style={{ width: 36, height: 36, border: `1px solid ${C.bd}`, borderRadius: 2, background: 'transparent', color: C.mut2, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={18} /></button>
            </div>
            <div className="overflow-y-auto flex-1">
              <div className="grid grid-cols-5 px-4 py-2 sticky top-0" style={{ background: C.s2, fontSize: 10, fontWeight: 800, letterSpacing: '0.14em', color: C.mut2 }}>
              ...
              {entries.length === 0
                ? <div className="py-10 text-center" style={{ color: C.mut2, fontSize: 14 }}>Brak wpisów z dziś</div>
                : entries.slice().reverse().map(e => {
                  ...
                  return (
                    <div key={e.id} className="grid grid-cols-5 px-4 py-3 items-center" style={{ borderTop: `1px solid ${C.bd}`, fontSize: 13 }}>
                      <span style={{ fontFamily: 'monospace', color: C.mut2 }}>{time}</span>
      ...
      {statsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="flex flex-col" style={{ width: 660, maxHeight: '80vh', background: C.s1, border: `1px solid ${C.bd2}`, borderRadius: 2 }}>
            <div className="flex items-center gap-4 px-6 py-4 flex-shrink-0" style={{ borderBottom: `1px solid ${C.bd}` }}>
              <BarChart3 size={24} style={{ color: C.grn }} />
              <span style={{ fontSize: 22, fontWeight: 900, color: C.ink, flex: 1 }}>Statystyki dnia</span>
              <button type="button" onClick={() => setStatsModal(false)} style={{ width: 36, height: 36, border: `1px solid ${C.bd}`, borderRadius: 2, background: 'transparent', color: C.mut2, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={18} /></button>
            </div>
            <div className="overflow-y-auto flex-1">
              <div className="grid grid-cols-4 sticky top-0" style={{ background: C.s2 }}>
                <div className="px-4 py-3" style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.14em', color: C.mut2 }}>PRACOWNIK</div>
                {(['taken', 'meat', 'yield'] as const).map(k => (
                  <button key={k} type="button"
                    ...
                    style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.14em', color: statsSort === k ? C.acc : C.mut2, background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase' }}>
                ...
                : workerStats.map(s => {
                  ...
                  return (
                    <div key={s.name} className="grid grid-cols-4 px-4 py-4 items-center" style={{ borderTop: `1px solid ${C.bd}` }}>
                      ...
                      <span style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, fontSize: 15, color: statsSort === 'taken' ? C.ink : C.mut2 }}>{fmtKg(s.taken, 1)} kg</span>
                      <span style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: 700, fontSize: 15, color: statsSort === 'meat' ? C.grn : C.mut2 }}>{fmtKg(s.meat, 1)} kg</span>
      ...
      {finishModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="flex flex-col gap-5 p-7" style={{ width: 480, background: C.s1, border: `1px solid ${C.bd2}`, borderRadius: 2 }}>
            <div className="flex items-center gap-4">
              <div style={{ width: 52, height: 52, border: `2px solid ${C.amb}`, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.amb }}><Flag size={26} /></div>
              <div>
                <div style={{ fontSize: 22, fontWeight: 900, color: C.ink }}>Zakończenie partii</div>
                <div style={{ fontSize: 13, color: C.mut2, fontFamily: 'monospace' }}>{pendingFinalize.length} wpisów · {fmtKg(finalizeTotalTaken, 1)} kg</div>
              </div>
            </div>
            {(['Grzbiety (kg)', 'Kości (kg)'] as const).map((label, i) => (
              <label key={label} className="flex flex-col gap-1.5">
                <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.16em', color: C.mut2 }}>{label}</span>
                <input type="number" min="0" step="0.01" value={i === 0 ? inputBacks : inputBones}
                  ...
                  style={{ height: 52, border: `1px solid ${C.bd2}`, borderRadius: 2, padding: '0 14px', fontSize: 22, fontFamily: 'monospace', fontWeight: 700, background: C.s2, color: C.ink, outline: 'none' }} />
              </label>
            ))}
            <div className="flex gap-3">
              <button type="button" onClick={() => setFinishModal(false)} style={{ flex: 1, height: 50, border: `1px solid ${C.bd}`, borderRadius: 2, background: 'transparent', color: C.mut2, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>Anuluj</button>
              <button type="button" onClick={handleFinishBatchConfirm} style={{ flex: 2, height: 50, border: 'none', borderRadius: 2, background: C.amb, color: '#000', fontSize: 16, fontWeight: 900, cursor: 'pointer' }}>Zatwierdź zakończenie</button>
            </div>
          </div>
        </div>
      )}
      ...
      {shiftModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.85)' }}>
          <div className="flex flex-col gap-5 p-7" style={{ width: 400, background: C.s1, border: `1px solid ${C.bd2}`, borderRadius: 2 }}>
            <div className="flex items-center gap-4">
              <div style={{ width: 52, height: 52, border: `2px solid ${C.red}`, borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', color: C.red }}><LogOut size={26} /></div>
              <div>
                <div style={{ fontSize: 22, fontWeight: 900, color: C.ink }}>Zakończyć zmianę?</div>
                <div style={{ fontSize: 13, color: C.mut2 }}>Sesja zostanie zamknięta.</div>
              </div>
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setShiftModal(false)} style={{ flex: 1, height: 50, border: `1px solid ${C.bd}`, borderRadius: 2, background: 'transparent', color: C.mut2, fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>Anuluj</button>
              <button type="button" onClick={handleCloseShift} disabled={closeLoading}
                className="flex items-center justify-center gap-3" style={{ flex: 2, height: 50, border: 'none', borderRadius: 2, background: C.red, color: C.wht, fontSize: 16, fontWeight: 900, cursor: 'pointer' }}>
