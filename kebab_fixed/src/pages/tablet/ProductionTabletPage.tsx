...
const STATUS_PILL: Record<'PLANNED'|'IN_PROGRESS'|'DONE', string> = {
  PLANNED:'bg-slate-900-light text-blue-600 border-brand-border',
...
function KpiBar({ plan, progress }: { plan:ProductionPlan; progress:ProgressMap }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 px-4 pt-3 pb-2 flex-shrink-0">
      {[
        {label:'Plan',         val:`${plannedSzt} szt`,   sub:`${fmtKg(plannedKg,0)} kg`, color:'text-slate-900',    border:'border-t-surface-4'},
        ...
        <div key={c.label} className={cn('bg-white rounded-xl border border-slate-200 shadow-card px-3 py-2.5 text-center border-t-2',c.border)}>
          <div className="text-[9px] font-bold uppercase tracking-wide text-slate-900-3 mb-0.5">{c.label}</div>
          <div className={cn('text-xl font-black tabular-nums leading-none',c.color)}>{c.val}</div>
          <div className="text-[10px] text-slate-900-3 font-semibold mt-0.5">{c.sub}</div>
        </div>
      ))}
    </div>
  )
...
function ColumnHeader() {
  return (
    <div className={cn('grid items-center px-3 py-2 bg-slate-50 border-b-2 border-slate-100',GRID,GAP)}>
      {['#','Szt×kg','Receptura','Partie','Tuleja','Klient','Waga','Status',''].map(h=>(
        <span key={h} className="text-[9px] font-bold text-slate-900-3 uppercase tracking-wide">{h}</span>
      ))}
    </div>
  )
}
...
function ProductionRow({ line, nr, prog, onClick, onEdit }: {
  ...
  return (
    <div className={cn('border-b-2 border-slate-100 last:border-b-0 border-l-4',accent,rowBg)}>
      <button onClick={onClick} disabled={isDone}
        className={cn('w-full text-left transition-colors select-none',isDone?'cursor-default':'active:bg-slate-50/50 hover:bg-slate-50/60 cursor-pointer')}>
        <div className={cn('grid items-center px-3 py-2.5',GRID,GAP)}>
          <span className="text-[10px] font-bold text-slate-900-3 tabular-nums text-center">{nr}</span>
          <div className="flex items-baseline justify-center gap-0.5">
            <span className="text-sm font-black text-slate-900 tabular-nums">{line.qty}</span>
            <span className="text-[10px] text-slate-900-4 mx-0.5">×</span>
            <span className="text-sm font-black text-blue-600 tabular-nums">{line.kgPerUnit}</span>
            <span className="text-[9px] text-slate-900-3 font-semibold">kg</span>
          </div>
          <span className="text-xs font-semibold text-slate-900 uppercase truncate">{line.recipeName}</span>
          <div className="flex flex-col items-center gap-0.5">
            {batchNos.length>0
              ? batchNos.map((n:string,i:number)=>(
                  <span key={i} className="text-[9px] font-bold font-mono text-blue-600 bg-slate-900-light border border-brand-border px-1 py-0.5 rounded whitespace-nowrap">{n}</span>
                ))
              : <span className="text-[9px] text-slate-900-4">—</span>
            }
          </div>
          <span className="text-xs font-bold text-slate-900-2 font-mono truncate text-center">{line.packagingName||'—'}</span>
          <span className="text-xs font-bold text-slate-900 truncate">{line.clientName||'—'}</span>
          <div className="text-right">
            <span className="text-sm font-black text-success tabular-nums">{line.totalKg}</span>
            <span className="text-[9px] text-slate-900-3 font-semibold ml-0.5">kg</span>
          </div>
          ...
          {isDone?<CheckCircle size={12} className="text-success"/>:<ChevronRight size={12} className="text-slate-900-4"/>}
        </div>
      </button>
        ...
        <div className="px-3 pb-2.5 pt-0">
          <div className="flex items-center gap-2 mb-1.5">
            <div className="flex-1 h-1.5 bg-slate-50 rounded-full overflow-hidden">
              <div className={cn('h-full rounded-full',pct>=100?'bg-success':pct>=50?'bg-slate-900':'bg-warn')} style={{width:`${pct}%`}}/>
            </div>
            <span className="text-[10px] font-bold tabular-nums text-slate-900-3 w-7 text-right">{Math.round(pct)}%</span>
          </div>
          {(prog?.workerEntries??[]).length>0&&(
            <div className="flex items-center gap-1.5 flex-wrap">
              {(prog?.workerEntries??[]).map((e,i)=>(
                <span key={i} className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-900-2">
                  <span className="w-4 h-4 rounded-full bg-slate-900-light text-blue-600 flex items-center justify-center text-[8px] font-black flex-shrink-0">
                    {e.workerName.split(' ')[0][0]}
                  </span>
                  {e.workerName.split(' ')[0]}
                  <span className="font-black text-slate-900">{e.pieces}</span>
                  <span className="text-slate-900-3">szt</span>
                </span>
              ))}
              <button onClick={e=>{e.stopPropagation();onEdit()}}
                className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-lg bg-white border border-slate-200 text-slate-900-3 hover:border-brand hover:text-blue-600 ml-auto">
...
function ProgressModal({ line, prog, mode, workers, onSave, onSaveEdit, onClose }: {
  ...
  if (mode==='add') return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
        <div className="mb-4 pb-4 border-b border-slate-100">
          <div className="text-xs font-bold text-slate-900-3 mb-1">Dodaj postęp</div>
          <div className="font-black text-slate-900 text-lg">{line.clientName||line.recipeName}</div>
          <div className="text-sm text-slate-900-3 mt-0.5">{line.qty}×{line.kgPerUnit}kg · {line.recipeName}</div>
          <div className="text-xs text-slate-900-3 mt-1">Wykonano <strong>{completed}</strong>/{line.qty} · Pozostało <strong className="text-warn">{rem}</strong></div>
        </div>
        <div className="mb-4">
          <div className="text-[11px] font-bold uppercase text-slate-900-3 mb-2">Pracownik *</div>
          {workers.length===0
            ? <div className="text-[12px] text-slate-900-3 bg-amber-50 border border-amber-200 p-3 rounded">Brak pracowników z rolą "Pracownik produkcji". Dodaj w panelu Pracownicy.</div>
                ...
                {workers.map(w=>{
                  ...
                  return (
                    <button key={w.id} onClick={()=>setWorker(w.id)}
                      className={cn('flex items-center gap-2 px-3 py-2.5 rounded-xl border-2 text-left transition-all',sel?'bg-slate-900 border-brand':'bg-white border-slate-200 hover:border-brand/40')}>
                      <div className={cn('w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-black',sel?'bg-white/20 text-white':'bg-slate-900-light text-blue-600')}>{init}</div>
                      <span className={cn('text-xs font-bold truncate',sel?'text-white':'text-slate-900')}>{w.name.split(' ')[0]}</span>
                    </button>
                  )
                })}
              </div>
        ...
        <div className="mb-5">
          <div className="text-[11px] font-bold uppercase text-slate-900-3 mb-2">Sztuk *</div>
          <div className="flex items-center justify-center gap-5">
            <button onClick={()=>setCount(c=>Math.max(1,c-1))} className="w-14 h-14 rounded-2xl bg-slate-50 border-2 border-slate-200 flex items-center justify-center active:scale-90"><Minus size={22} className="text-slate-900-2"/></button>
            <div className="flex flex-col items-center min-w-[80px]">
              <div className="text-6xl font-black text-slate-900 tabular-nums leading-none text-center">{count}</div>
              <div className="text-xs text-slate-900-3 font-semibold mt-1">= {count*line.kgPerUnit} kg</div>
            </div>
            <button onClick={()=>setCount(c=>Math.min(Math.max(rem,1),c+1))} className="w-14 h-14 rounded-2xl bg-slate-900 flex items-center justify-center active:scale-90"><Plus size={22} className="text-white"/></button>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 h-12 rounded-2xl bg-slate-50 font-bold text-sm text-slate-900-2 border border-slate-200">Anuluj</button>
          <button onClick={()=>{const w=workers.find(x=>x.id===worker);if(w)onSave(w.id,w.name,count)}} disabled={!worker}
            className="flex-1 h-12 rounded-2xl bg-slate-900 text-white font-bold text-sm disabled:opacity-40 active:scale-[.97]">Zapisz {count} szt</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl w-full max-w-sm p-6" onClick={e=>e.stopPropagation()}>
        ...
        {editEntries.length===0 ? <div className="text-center py-6 text-sm text-slate-900-3">Brak wpisów</div> : (
            ...
            {editEntries.map((entry,idx)=>{
              ...
              return (
                <div key={idx} className="flex items-center gap-3 bg-slate-50 rounded-xl px-3 py-2.5">
                  <div className="w-8 h-8 rounded-full bg-slate-900-light text-blue-600 flex items-center justify-center text-xs font-black flex-shrink-0">{init}</div>
                  <span className="text-sm font-semibold text-slate-900 flex-1 truncate">{entry.workerName.split(' ')[0]}</span>
                  <div className="flex items-center gap-2">
                    <button onClick={()=>setEditEntries(p=>p.map((e,i)=>i===idx?{...e,pieces:Math.max(0,e.pieces-1)}:e))} className="w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center"><Minus size={14} className="text-slate-900-2"/></button>
                    <input type="number" value={entry.pieces} onChange={e=>setEditEntries(p=>p.map((en,i)=>i===idx?{...en,pieces:Math.max(0,parseInt(e.target.value)||0)}:en))}
                      className="w-14 text-center text-lg font-black text-slate-900 tabular-nums bg-white border-2 border-slate-200 rounded-lg focus:outline-none focus:border-brand [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"/>
                    <button onClick={()=>setEditEntries(p=>p.map((e,i)=>i===idx?{...e,pieces:e.pieces+1}:e))} className="w-8 h-8 rounded-lg bg-slate-900 flex items-center justify-center"><Plus size={14} className="text-white"/></button>
                    <span className="text-xs text-slate-900-3 font-semibold w-6">szt</span>
                  </div>
                </div>
              )
            })}
            <div className="flex justify-between text-xs font-semibold text-slate-900-3 px-1 pt-1">
              <span>Łącznie:</span>
              <span className="font-black text-slate-900">{editEntries.reduce((s,e)=>s+e.pieces,0)} szt · {editEntries.reduce((s,e)=>s+e.pieces,0)*line.kgPerUnit} kg</span>
            </div>
          </div>
        )}
        <div className="flex gap-3">
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl w-full max-w-md p-6" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-black text-slate-900">Zakończ dzień produkcji</h3>
            <p className="text-sm text-slate-900-3">Plan: {plan.planNo}</p>
          </div>
        </div>

        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 mb-4 space-y-1.5 text-[12px]">
          <div className="flex justify-between"><span className="text-slate-900-3">Wyprodukowano:</span><span className="font-bold text-green-700">{totalDone} szt · {fmtKg(totalKgDone)} kg</span></div>
          <div className="flex justify-between"><span className="text-slate-900-3">Gotowe pozycje:</span><span className="font-semibold">{doneLines.length}</span></div>
          ...
          {plannedLines.length>0&&<div className="flex justify-between"><span className="text-slate-900-3">Niezrealizowane:</span><span className="font-semibold text-slate-900-3">{plannedLines.length}</span></div>}
        </div>
...
function PlanPicker({ plans, onSelect }: { plans:ProductionPlan[]; onSelect:(id:string)=>void }) {
  ...
  if (all.length===0) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
      <Factory size={48} className="text-slate-900-5 mb-4"/>
      <h2 className="text-2xl font-black text-slate-900 mb-2">Brak aktywnych planów</h2>
      <p className="text-base text-slate-900-3">Biuro musi aktywować plan produkcji.</p>
    </div>
  )

  return (
    <div className="max-w-lg mx-auto px-5 py-6">
      <h2 className="text-xl font-black text-slate-900 mb-4">Wybierz plan produkcji</h2>
      <div className="space-y-3">
        {all.map(p=>(
          <button key={p.id} onClick={()=>onSelect(p.id)}
            className="w-full text-left bg-white border-2 border-slate-200 rounded-2xl p-4 hover:border-brand hover:shadow-md active:scale-[.99]">
            <div className="flex items-center justify-between">
              <div>
                <div className="font-mono font-bold text-blue-600">{p.planNo}</div>
                <div className="text-[12px] text-slate-900-3 mt-0.5">{p.planDate} · {p.lines.length} poz. · {fmtKg(p.totalKg,0)} kg · {p.totalUnits} szt</div>
              </div>
              <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${p.status==='active'?'bg-amber-50 text-amber-700':'bg-slate-50 text-slate-900-3'}`}>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-4 pt-3 pb-1 flex items-center justify-between flex-shrink-0">
        <div>
          <div className="font-mono text-sm font-bold text-blue-600">{selectedPlan.planNo}</div>
          <div className="text-[11px] text-slate-900-3">{selectedPlan.planDate} · {selectedPlan.lines.length} pozycji</div>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={()=>{setSelectedPlanId(null);setProgress({})}}
            className="text-[12px] font-semibold text-slate-900-3 border border-slate-200 px-3 py-1.5 rounded-lg hover:border-brand hover:text-blue-600 flex items-center gap-1.5">
      ...
      <div className="flex-1 overflow-y-auto mx-4 mb-4 bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden">
        <ColumnHeader/>
