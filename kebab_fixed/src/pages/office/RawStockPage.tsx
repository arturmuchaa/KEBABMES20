  ...
  return (
    <Modal open={true} onClose={onClose} title={title} size="lg">
      <div className="space-y-4">
        <div className="bg-slate-900-light border-2 border-brand rounded-xl p-4">
          <div className="text-[10px] font-bold text-blue-600 uppercase mb-1">Nasza partia</div>
          <div className="text-3xl font-black font-mono text-blue-600">{batch?.internalBatchNo || item?.rawBatchNo || '—'}</div>
        </div>
        <div className="flex items-center gap-2 text-slate-900-3">
        ...
        <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
          <tbody>
            <tr className="border-b border-slate-200">
              <td className="py-2 font-semibold text-slate-900-3 w-1/3">Dostawca:</td>
              <td className="py-2 font-bold">{batch?.supplierName || '—'}</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="py-2 font-semibold text-slate-900-3">Nr partii dostawcy:</td>
              <td className="py-2 font-mono font-bold">{batch?.supplierBatchNo || '—'}</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="py-2 font-semibold text-slate-900-3">Data uboju:</td>
              <td className="py-2">{batch?.slaughterDate ? fmtDatePl(batch.slaughterDate) : '—'}</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="py-2 font-semibold text-slate-900-3">Data przyjęcia:</td>
              <td className="py-2">{batch?.receivedDate ? fmtDatePl(batch.receivedDate) : '—'}</td>
            </tr>
            <tr className="border-b border-slate-200">
              <td className="py-2 font-semibold text-slate-900-3">Data ważności:</td>
              <td className="py-2">
                {batch?.expiryDate ? (
            ...
            {session && (
              <>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <td className="py-2 font-semibold text-slate-900-3" colSpan={2}>
                    <span className="text-[10px] uppercase tracking-wide">Sesja rozbioru</span>
                  </td>
                </tr>
                <tr className="border-b border-slate-200">
                  <td className="py-2 font-semibold text-slate-900-3">Nr sesji:</td>
                  <td className="py-2 font-mono">{session.sessionNo}</td>
                </tr>
                <tr className="border-b border-slate-200">
                  <td className="py-2 font-semibold text-slate-900-3">Pracownik:</td>
                  <td className="py-2">{session.workerName}</td>
                </tr>
                <tr className="border-b border-slate-200">
                  <td className="py-2 font-semibold text-slate-900-3">Data rozbioru:</td>
            ...
            {item && (
              <>
                ...
                <tr className="border-b border-slate-200">
                  <td className="py-2 font-semibold text-slate-900-3">Nr partii mięsa:</td>
                  <td className="py-2 font-mono font-bold text-blue-600">{item.lotNo}</td>
                </tr>
                <tr className="border-b border-slate-200">
                  <td className="py-2 font-semibold text-slate-900-3">Ilość początkowa:</td>
                  <td className="py-2 font-bold">{fmtKg(item.kgInitial, 1)} kg</td>
                </tr>
                <tr>
                  <td className="py-2 font-semibold text-slate-900-3">Ilość dostępna:</td>
...
export function RawStockPage() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <PageHeader title="Magazyn surowca" subtitle="Partie mięsa gotowe do produkcji" />
      <Card className="p-5">
          ...
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
              className={cn('p-4 rounded-xl border-2 text-center transition-all',
                activeTab === tab.key ? 'border-brand bg-slate-900-light' : 'border-slate-200 bg-slate-50 hover:border-brand/30')}>
              <div className={cn('flex justify-center mb-1', activeTab === tab.key ? 'text-blue-600' : 'text-slate-900-3')}>{tab.icon}</div>
              <div className={cn('text-lg font-bold', activeTab === tab.key ? 'text-blue-600' : 'text-slate-900')}>{tab.val}</div>
              <div className={cn('text-[10px] font-semibold uppercase tracking-wide', activeTab === tab.key ? 'text-blue-600' : 'text-slate-900-3')}>{tab.label}</div>
            </button>
          ))}
        </div>
      </Card>
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-slate-50/60">
                      <td className="px-3 py-2.5">
                        <div className="font-mono text-[11px] text-slate-900-3">{m.lotNo}</div>
                        <div className="font-mono font-bold text-blue-600">{batch?.internalBatchNo || m.rawBatchNo}</div>
                        {batch?.supplierName && (
                          <div className="text-[10px] text-slate-900-3">{batch.supplierName}</div>
                        )}
                        {batch?.supplierBatchNo && (
                          <div className="text-[10px] text-slate-900-4">nr: {batch.supplierBatchNo}</div>
                        )}
                        {batch?.slaughterDate && (
                          <div className="text-[10px] text-slate-900-4">ubój: {fmtDatePl(batch.slaughterDate)}</div>
                        )}
                      </td>
                      <td className="px-3 py-2.5">
                        <div className="font-bold text-success text-base">{fmtKg(m.kgAvailable, 2)} kg</div>
                        <div className="text-[10px] text-slate-900-3">z {fmtKg(m.kgInitial, 2)} kg</div>
                      </td>
                      <td className="px-3 py-2.5">
                        <div className="text-slate-900-3">Prod: {fmtDatePl(m.productionDate)}</div>
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className="text-slate-900-3">Ważn:</span>
                ...
                {backsItems.map((s, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-slate-50/60">
                      <td className="px-3 py-2 font-mono font-bold text-blue-600">{batch?.internalBatchNo || s.rawBatchNo}</td>
                      <td className="px-3 py-2 text-slate-900-3">{batch?.supplierName || '—'}</td>
                      <td className="px-3 py-2 font-bold">{fmtKg(s.kgBacks, 2)} kg</td>
                      <td className="px-3 py-2 text-slate-900-3">{fmtDatePl(s.createdAt?.slice(0,10) || '')}</td>
                    </tr>
                  )
                })}
              </tbody>
