    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <label className="block text-[10px] font-bold text-slate-900-3 uppercase tracking-wide mb-1">Nazwa *</label>
          <input value={form.name} onChange={e => set('name', e.target.value)}
            placeholder="np. Tuleja metal 65cm" className="w-full h-9 px-3 text-sm border border-slate-200 focus:outline-none focus:border-brand bg-slate-50" />
        </div>
        <div>
          <label className="block text-[10px] font-bold text-slate-900-3 uppercase tracking-wide mb-1">Typ</label>
        ...
        <div>
          <label className="block text-[10px] font-bold text-slate-900-3 uppercase tracking-wide mb-1">Jednostka</label>
        ...
        <div>
          <label className="block text-[10px] font-bold text-slate-900-3 uppercase tracking-wide mb-1">Ilość *</label>
...
export function PackagingPage() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <PageHeader title="Opakowania" subtitle="Magazyn opakowań" />
      <div className="flex gap-3 items-center">
        <div className="flex gap-1">
          {([['all','Wszystkie'],['tuleja','Tuleje'],['opakowanie','Opakowania']] as [string,string][]).map(([k,l]) => (
            <button key={k} onClick={() => setTab(k as any)}
              className={`px-3 py-1.5 rounded text-[12px] font-semibold border transition-all ${tab===k?'bg-slate-900 text-white border-brand':'bg-slate-50 text-slate-900-3 border-slate-200 hover:border-brand/40'}`}>
              {l}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl">
        <div className="px-4 py-2.5 border-b border-slate-200 flex items-center justify-between">
          <span className="text-[13px] font-semibold text-slate-900">{filtered.length} pozycji</span>
          <span className="text-[12px] text-slate-900-3">Łącznie: {totalAvail.toFixed(0)} szt/kg dostępne</span>
        </div>
        {loading ? (
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(item => (
                <tr key={item.id} className="hover:bg-slate-50/60">
                  <td className="px-3 py-2.5 font-mono text-slate-900-3">{item.code}</td>
                  <td className="px-3 py-2.5 font-semibold text-slate-900">{item.name}</td>
                  <td className="px-3 py-2.5">
                    <span className="text-[10px] bg-slate-50 text-slate-900-3 px-1.5 py-0.5 rounded font-semibold">
                      {TYPE_LABELS[item.type]}
                    </span>
                  </td>
                  <td className="px-3 py-2.5">
                    <span className={`font-bold ${item.kgAvailable > 0 ? 'text-success' : 'text-slate-900-4'}`}>
                      {item.kgAvailable} {item.unit}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-slate-900-3">{item.kgUsed} {item.unit}</td>
                  <td className="px-3 py-2.5 text-slate-900-3">{item.supplierName || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
