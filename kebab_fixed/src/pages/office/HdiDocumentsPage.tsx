...
import {
  Search, ChevronDown, ChevronUp, ChevronsUpDown, X, Printer, FileText, ExternalLink,
} from 'lucide-react'
...
export function HdiDocumentsPage() {
  ...
  // Domyślnie: po numerze, od najnowszego.
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <CardContent className="flex flex-col items-center justify-center py-16 gap-2">
            ...
            <CardTitle className="text-sm font-medium text-muted-foreground">Brak dokumentów HDI</CardTitle>
            <CardDescription>HDI wystawisz z listy zamówień (przycisk „HDI").</CardDescription>
          </CardContent>
        ) : list.length === 0 ? (
          ...
          <div className="overflow-auto max-h-[calc(100vh-12rem)]">
            <table className="w-full text-xs tabular-nums">
              <thead className="sticky top-0 z-10 bg-surface-2/95 backdrop-blur-sm border-b-2 border-surface-4">
                <tr>
                  {[
                    { col: 'number'     as SortCol, label: 'Numer HDI',       align: 'left'  },
                ...
                {list.map((r, idx) => (
                  ...
                  >
                    <td className="px-2.5 py-2 whitespace-nowrap font-bold text-ink">
                      ...
                      {r.incomplete && (
                        <span
                          className="ml-1.5 inline-block px-1 py-px rounded text-[9px] font-semibold bg-orange-50 text-orange-700 border border-orange-200 align-middle"
                          title="Wystawiony na stan faktyczny — niekompletny względem zamówienia"
                        >
                          niekompletny
                        </span>
                      )}
