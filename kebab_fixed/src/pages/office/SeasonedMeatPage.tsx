      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            { label: 'Dostępne',      val: `${fmtKg(batch.kgAvailable)} kg`, color: 'text-slate-900' },
            { label: 'Ważność',       val: fmtDatePl(batch.expiryDate), color: 'text-slate-900' },
          ].map(k => (
            <div key={k.label} className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
              <div className="text-[10px] font-bold text-slate-900-4 uppercase mb-0.5">{k.label}</div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3">
          <div className="text-[10px] font-bold text-slate-900-4 uppercase mb-2">Łańcuch partii</div>
          <div className="flex items-center gap-1.5 flex-wrap text-[12px]">
            ...
            <ChevronRight size={13} className="text-slate-900-4"/>
            ...
            <ChevronRight size={13} className="text-slate-900-4"/>
            <span className="font-mono font-black text-blue-600 bg-slate-900-light border border-brand-border px-2 py-1 rounded">{batch.batchNo}</span>
          </div>
        </div>
        ...
        <div className="border border-slate-200 rounded-lg divide-y divide-slate-100 text-[12px]">
          {[
            ...
            <div key={r.label} className="grid grid-cols-[140px_1fr] gap-2 px-3 py-2.5">
              <span className="text-slate-900-3 font-semibold">{r.label}</span>
              <span className={r.mono ? 'font-mono font-bold text-blue-600' : 'font-semibold text-slate-900'}>{r.val}</span>
            </div>
          ))}
        </div>
        ...
        {batch.meatLots.length > 0 && (
          <div>
            <div className="text-[10px] font-bold text-slate-900-4 uppercase mb-2">Partie mięsa (Z/S)</div>
            <div className="border border-slate-200 rounded-lg divide-y divide-slate-100">
              {batch.meatLots.map(l => (
                <div key={l.meatLotId} className="flex items-center gap-3 px-3 py-2.5 text-[12px]">
                  ...
                  {l.rawBatchNo && <span className="text-slate-900-3">← {l.rawBatchNo}</span>}
                  <span className="ml-auto font-semibold text-slate-900">{fmtKg(l.kgPlanned)} kg</span>
                  {l.expiryDate && <span className="text-[11px] text-slate-900-4">do: {fmtDatePl(l.expiryDate)}</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        {batch.slaughterDates.length > 0 && (
          <div className="text-[12px] text-slate-900-3">
...
export function SeasonedMeatPage() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <PageHeader title="Mięso przyprawione" subtitle="Magazyn półproduktów — partie PW" />
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label:'Partie dostępne', val: batches.length, color:'text-slate-900' },
          { label:'Łącznie kg',      val: `${fmtKg(totalAvail)} kg`, color:'text-green-700' },
          { label:'Alerty',          val: critical.length, color: critical.length > 0 ? 'text-red-600' : 'text-slate-900-4' },
        ].map(k => (
      ...
      <div className="bg-white border border-slate-200 rounded-xl">
        <div className="px-4 py-2.5 border-b border-slate-200 flex items-center gap-2">
          <Beef size={13} className="text-slate-900-3" />
          <span className="text-[13px] font-semibold text-slate-900">Dostępne partie (FEFO)</span>
        </div>
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-blue-600">{b.batchNo}</span>
                        <span className="text-[11px] text-slate-900-3">{b.recipeName}</span>
                        {b.productTypeName && (
                      ...
                      <div className="flex items-center gap-1 mt-1 flex-wrap">
                        {b.rawBatchNos.map(n => (
                          <span key={n} className="text-[10px] font-mono bg-slate-50 text-slate-900-3 px-1.5 py-0.5 rounded">{n}</span>
                        ))}
                        {b.rawBatchNos.length > 0 && <ChevronRight size={10} className="text-slate-900-5" />}
                        {b.meatLots.map(l => (
                          <span key={l.meatLotId} className="text-[10px] font-mono bg-green-500/15 text-green-400 px-1.5 py-0.5 rounded">{l.meatLotNo}</span>
                        ))}
                        {b.meatLots.length > 0 && <ChevronRight size={10} className="text-slate-900-5" />}
                        <span className="text-[10px] font-mono font-bold bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">{b.batchNo}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-[11px] font-medium text-blue-600 border border-brand/30 px-2 py-1 rounded hover:bg-blue-50"
                  ...
                  {isExp && (
                    <div className="px-4 pb-3 bg-slate-50 border-t border-slate-200">
                      <div className="grid grid-cols-2 gap-3 mt-2 text-[12px]">
                        <div>
                          <div className="text-[10px] font-bold text-slate-900-4 uppercase mb-1">Masowanie</div>
                          <div>Zlecenie: <span className="font-mono font-bold text-blue-600">{b.mixingOrderNo}</span></div>
                        ...
                        <div>
                          <div className="text-[10px] font-bold text-slate-900-4 uppercase mb-1">Ćwiartki (surowiec)</div>
                          ...
                          {b.rawBatchNos?.length === 0 && b.meatLots.every((l: any) => !l.rawBatchNo) && <div className="text-slate-900-4">—</div>}
                          {b.slaughterDates.length > 0 && (
                            <div className="text-slate-900-3 mt-1">
      ...
      {allBatches.filter(b => b.status === 'depleted').length > 0 && (
        <div className="bg-white border border-slate-200 rounded-xl">
          <div className="px-4 py-2.5 border-b border-slate-200">
            <span className="text-[13px] font-semibold text-slate-900">Historia — wykorzystane partie</span>
          </div>
          <table className="w-full text-[12px]">
            <tbody className="divide-y divide-slate-100">
              {allBatches.filter(b => b.status === 'depleted').map(b => (
                <tr key={b.id} className="opacity-60">
                  <td className="px-3 py-2 font-mono text-slate-900-3">{b.batchNo}</td>
                  <td className="px-3 py-2">{b.recipeName}</td>
                  <td className="px-3 py-2 font-mono text-[11px] text-slate-900-4">
                    {b.rawBatchNos.join(', ') || '—'}
                  </td>
                  <td className="px-3 py-2 text-slate-900-4">{fmtKg(b.kgProduced)} kg</td>
                  <td className="px-3 py-2">
                    <button onClick={() => setTraceBatch(b)}
                      className="text-[11px] text-slate-900-3 border border-slate-200 px-1.5 py-0.5 rounded hover:text-blue-600">
