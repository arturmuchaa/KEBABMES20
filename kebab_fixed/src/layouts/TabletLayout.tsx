...
function Clock() {
  ...
  return (
    <div className="text-right leading-tight">
      <div className="text-base font-bold font-mono text-slate-900 tabular-nums">{clock}</div>
      <div className="text-[11px] text-slate-900-3 font-medium">{date}</div>
    </div>
  )
}
...
export function TabletLayout() {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <header className="flex items-center justify-between px-5 py-3 bg-white border-b border-slate-200 flex-shrink-0 shadow-card">
        <div className="flex items-center gap-3">
          {/* Mały niebieski akcent zamiast kolorowego bloku */}
          <div className="w-1 h-8 bg-brand rounded-full" />
          <div>
            <div className="text-base font-bold text-slate-900 leading-tight">{meta.title}</div>
            <div className="text-[11px] text-slate-900-3 font-medium">{meta.sub}</div>
          </div>
        </div>
        <Clock />
      </header>
