...
async function req<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  if (res.status === 401) {
    ...
    // Każde samodzielne wejście kiosku musi tu być wymienione. `produkcja`
    // i `masowanie` wypadły z tej listy przy dodawaniu kolejnych stanowisk
    // i 401 wyrzucał tam operatora do pełnego MES-a — tego samego błędu, który
    // naprawiono dla rozbioru w 2026-07.
    const standaloneKiosk = ['rozbior-v', 'kiosk', 'produkcja', 'masowanie']
      .some(entry => location.pathname.includes(entry))
    if (standaloneKiosk) {
      location.reload()
...
export const hdiApi = {
  ...
  generateFromWz: (wzId: string) =>
    post<{ id: string; number: string; status: string; incomplete: boolean; totals: { qty: number; kg: number } }>(`/hdi/z-wz?wz_id=${encodeURIComponent(wzId)}`, {}),
...
export const rozrachunkiApi = {
  ...
  dostawa: (orderId: string, ceny: Record<string, number>) =>
    post<any>(`/rozrachunki/dostawa/${orderId}`, { ceny }),
}
