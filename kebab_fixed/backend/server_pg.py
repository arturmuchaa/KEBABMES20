...
# ─── VIES API — EC VIES REST (bez auth, darmowy) ───────────────
@app.get("/api/vies/lookup")
def vies_lookup(vat: str):
    """Weryfikacja VAT-UE przez EC VIES REST API."""
    ...
    cc = vat[:2]; vn = vat[2:]
    if not cc.isalpha() or len(vn) < 2:
        raise HTTPException(400, "Nieprawidlowy format VAT-UE (np. DE123456789)")

    url = f"https://ec.europa.eu/taxation_customs/vies/rest-api/ms/{cc}/vat/{vn}"
