...
_UPDATES_DIR = settings.desktop_updates_dir
...
def _load_meta() -> dict | None:
    ...
    try:
        ...
        logger.exception("desktop_updates.meta_invalid", extra={"error": str(exc)})
...
def _manifest_payload(request: Request, meta: dict) -> dict:
    return {
        ...
        "url": str(request.url_for("desktop_update_download", filename=meta["filename"])),
        "signature": meta["signature"],
    }


@router.get("/api/desktop-updates/latest.json")
def desktop_update_manifest(request: Request):
    meta = _load_meta()
    if not meta:
...
@router.get("/api/desktop-updates/latest-installer")
def desktop_update_latest_installer(request: Request):
    meta = _load_meta()
    if not meta:
        raise HTTPException(404, "Brak opublikowanej aktualizacji")
    return RedirectResponse(url=str(request.url_for("desktop_update_download", filename=meta["filename"])))


@router.get("/api/desktop-updates/download/{filename}", name="desktop_update_download")
def desktop_update_download(filename: str):
...
@router.post("/api/admin/desktop-updates/publish", dependencies=[Depends(require_admin)])
async def publish_desktop_update(
    request: Request,
    version: str = Query(...),
    signature: str = Query(...),
    filename: str = Query(...),
    notes: str = Query("Nowa wersja Kebab MES"),
    pub_date: str = Query(""),
):
    safe_name = _safe_name(filename)
    ...
    meta = {
        ...
        "filename": safe_name,
        "notes": notes.strip() or f"Nowa wersja Kebab MES {version.strip()}",
    ...
    return {
        ...
        "downloadUrl": str(request.url_for("desktop_update_latest_installer")),
    }
