"""Manifest aktualizacji dla samodzielnego kiosku Tauri "Rozbiór HMI v7".
...
config, osobny cykl wydań rozbior-v7-*.tag), więc musi mieć własny manifest.
...
router = APIRouter(tags=["desktop-updates-rozbior-v7"])

_UPDATES_DIR = settings.desktop_updates_dir / "rozbior-v7"
...
def _load_meta() -> dict | None:
    ...
    try:
        ...
        logger.exception("desktop_updates_rozbior_v7.meta_invalid", extra={"error": str(exc)})
...
def _manifest_payload(request: Request, meta: dict) -> dict:
    return {
        ...
        "url": str(request.url_for("desktop_update_rozbior_v7_download", filename=meta["filename"])),
        "signature": meta["signature"],
    }


@router.get("/api/desktop-updates/rozbior-v7/latest.json")
def desktop_update_rozbior_v7_manifest(request: Request):
    meta = _load_meta()
    if not meta:
        return Response(status_code=204)
    bundle = _UPDATES_DIR / meta["filename"]
    if not bundle.is_file():
        logger.warning("desktop_updates_rozbior_v7.bundle_missing", extra={"filename": meta["filename"]})
        return Response(status_code=204)
    return _manifest_payload(request, meta)


@router.get("/api/desktop-updates/rozbior-v7/latest-installer")
def desktop_update_rozbior_v7_latest_installer(request: Request):
    meta = _load_meta()
    if not meta:
        raise HTTPException(404, "Brak opublikowanej aktualizacji")
    return RedirectResponse(url=str(request.url_for("desktop_update_rozbior_v7_download", filename=meta["filename"])))


@router.get("/api/desktop-updates/rozbior-v7/download/{filename}", name="desktop_update_rozbior_v7_download")
def desktop_update_rozbior_v7_download(filename: str):
...
@router.post("/api/admin/desktop-updates/rozbior-v7/publish", dependencies=[Depends(require_admin)])
async def publish_desktop_update_rozbior_v7(
    request: Request,
    version: str = Query(...),
    signature: str = Query(...),
    filename: str = Query(...),
    ...
    logger.info("desktop_updates_rozbior_v7.published", extra={"version": meta["version"], "installer": safe_name})
    return {
        "ok": True,
        "manifest": _manifest_payload(request, meta),
        "downloadUrl": str(request.url_for("desktop_update_rozbior_v7_latest_installer")),
    }
