"""Manifest aktualizacji dla samodzielnego kiosku Tauri "Produkcja HMI".
...
config, osobny cykl wydań produkcja-*.tag), więc musi mieć własny manifest.
...
router = APIRouter(tags=["desktop-updates-produkcja"])

_UPDATES_DIR = settings.desktop_updates_dir / "produkcja"
...
def _load_meta() -> dict | None:
    ...
    try:
        ...
        logger.exception("desktop_updates_produkcja.meta_invalid", extra={"error": str(exc)})
...
def _manifest_payload(request: Request, meta: dict) -> dict:
    return {
        ...
        "url": str(request.url_for("desktop_update_produkcja_download", filename=meta["filename"])),
        "signature": meta["signature"],
    }


@router.get("/api/desktop-updates/produkcja/latest.json")
def desktop_update_produkcja_manifest(request: Request):
    meta = _load_meta()
    if not meta:
        return Response(status_code=204)
    bundle = _UPDATES_DIR / meta["filename"]
    if not bundle.is_file():
        logger.warning("desktop_updates_produkcja.bundle_missing", extra={"filename": meta["filename"]})
        return Response(status_code=204)
    return _manifest_payload(request, meta)


@router.get("/api/desktop-updates/produkcja/latest-installer")
def desktop_update_produkcja_latest_installer(request: Request):
    meta = _load_meta()
    if not meta:
        raise HTTPException(404, "Brak opublikowanej aktualizacji")
    return RedirectResponse(url=str(request.url_for("desktop_update_produkcja_download", filename=meta["filename"])))


@router.get("/api/desktop-updates/produkcja/download/{filename}", name="desktop_update_produkcja_download")
def desktop_update_produkcja_download(filename: str):
...
@router.post("/api/admin/desktop-updates/produkcja/publish", dependencies=[Depends(require_admin)])
async def publish_desktop_update_produkcja(
    request: Request,
    version: str = Query(...),
    signature: str = Query(...),
    filename: str = Query(...),
    notes: str = Query("Nowa wersja HMI Produkcja"),
    pub_date: str = Query(""),
):
    safe_name = _safe_name(filename)
    ...
    meta = {
        ...
        "filename": safe_name,
        "notes": notes.strip() or f"Nowa wersja HMI Produkcja {version.strip()}",
    ...
    logger.info("desktop_updates_produkcja.published", extra={"version": meta["version"], "installer": safe_name})
    return {
        "ok": True,
        "manifest": _manifest_payload(request, meta),
        "downloadUrl": str(request.url_for("desktop_update_produkcja_latest_installer")),
    }


@router.get("/api/desktop-updates/produkcja/versions")
def desktop_update_produkcja_versions():
...
@router.post("/api/desktop-updates/produkcja/rollback")
def desktop_update_produkcja_rollback(body: dict = Body(...)):
    ...
    _META_FILE.write_text(json.dumps(meta, ensure_ascii=True, indent=2), encoding="utf-8")
    logger.warning("desktop_updates_produkcja.rollback", extra={"restored": version})
